                                ...
                                <div class="card-content">
                                    ...
                                    <div class="material-datatables">
                                        <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                            ...
                                            <tbody>
                                             <!-- request 영역으로 전달받은 챕터 목록 list를 el태그 형식으로 출력 한다. -->
                                            <c:forEach var = "list" items = "${playChapterList}" >
                                                <tr>
                                                    <td><p class="text-info">${list.order_num }</p></td>
                                                    <td><a href="playChapterViewAction.ac?playlist_num=${list.playlist_num }&category=${list.category }&order_num=${list.order_num }&lecture_level=${list.lecture_level }&lecture_level=${list.lecture_image }&id=<%=session.getAttribute("id")  %>">${list.chapter_subject }</a></td>
                                                    ...
                                                    <%if(session.getAttribute("id")!=null) {
                                                    	if(session.getAttribute("id").equals("admin")){%>
                                                    	<td><a href="PlayListModifyFormAction.ac?playlist_num=${list.playlist_num }" class="btn btn-rose btn-fill">챕터 수정</a></td>
                                                    	<%}else{ %>
                                                    		 <td>
